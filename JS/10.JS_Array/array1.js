...
/*
    배열의 요소 추가
    - 배열[인덱스] = 값 : 요소 추가 가능, 특정 지정한 인덱스 값이 존재하면 변경
...
/*
    - for-in 방식은 index를 사용하여 한번더 접근을 해야하기 때문에 단순히 읽어드리는 작업만 할 때는 적합하지 않을 수 있음
...
/*
    includes(element) :
    - 배열에 전달된 매개변수 요소에 존재하면 true, 존재하지 않으면 false 반환
*/
...
/*
    push(element) :
    - 배열의 마지막 인덱스에 요소를 추가하과 변경된 길이를 반환
*/
...
/*
    splice(start, eleteCount(제거할 개수), items...) :
    - 배열의 요소를 제거하과 제거한 위치에 요소를 갈아끼운 배열을 반환
*/
